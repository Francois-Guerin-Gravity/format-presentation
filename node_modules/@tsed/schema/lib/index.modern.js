...
import { Store, decoratorTypeOf, nameOf, isFunction, cleanObject, isPromise, isClass, isSymbol, isPrimitiveOrPrimitiveClass, isClassObject, isDate, isCollection, classOf, isArrowFn, primitiveOf, isArray, Hooks, uniq, isObject, uniqBy, deepMerge, toMap, Entity, DecoratorTypes, descriptorOf, Metadata, isPrimitiveClass, ancestorsOf, mergeReducerBuilder, UnsupportedDecoratorType, decorateMethodsOf, isString, isPlainObject, isNumber, useDecorators, StoreSet } from '@tsed/core';
...
const isRedirectionStatus = code => Number(code) && 300 <= Number(code) && Number(code) < 400;
...
var _redirection = /*#__PURE__*/_classPrivateFieldLooseKey("redirection");
...
class JsonOperation extends JsonMap {
  constructor(obj = {}) {
    ...
    Object.defineProperty(this, _redirection, {
      writable: true,
      value: void 0
    });
    this.operationPaths = new Map();
    _classPrivateFieldLooseBase(this, _redirection)[_redirection] = false;
  }
  ...
  getStatus() {
    return _classPrivateFieldLooseBase(this, _status)[_status] || 200;
  }

  setRedirection(status = 302) {
    _classPrivateFieldLooseBase(this, _redirection)[_redirection] = true;
    _classPrivateFieldLooseBase(this, _status)[_status] = status;
    return this;
  }

  isRedirection(status) {
    if (_classPrivateFieldLooseBase(this, _redirection)[_redirection]) {
      if (status) {
        return isRedirectionStatus(status);
      }
    }

    return _classPrivateFieldLooseBase(this, _redirection)[_redirection];
  }

  addResponse(statusCode, response) {
    if ((isSuccessStatus(statusCode) || isRedirectionStatus(statusCode)) && !_classPrivateFieldLooseBase(this, _status)[_status]) {
...
class ReturnDecoratorContext extends DecoratorContext {
  constructor({
    ...
    super();
    this.methods = ["contentType", "description", "examples", "type", "status", "of", "nested", "header", "headers", "schema", "title", "groups", "location"];
    this.model(model);
    this.status(status);
  }
...
function Redirect(...args) {
  ...
  return useDecorators(Returns(status).Location(url, meta), JsonEntityFn(entity => {
    var _entity$operation;

    (_entity$operation = entity.operation) == null ? void 0 : _entity$operation.setRedirection(status);
  }));
}
...
function Location(location, meta) {
...
export { AdditionalProperties, AllOf, Allow, Any, AnyOf, ArrayOf, CollectionContains, CollectionOf, Const, Consumes, ContentType, CustomKey, CustomKeys, DateFormat, DateTime, Default, Deprecated, Description, Email, Enum, Example, ExclusiveMaximum, ExclusiveMinimum, For, Format, ForwardGroups, GenericOf, Generics, Groups, HTTP_STATUS_MESSAGES, Header, Hidden, Ignore, In, Integer, JSON_TYPES, JsonEntityComponent, JsonEntityFn, JsonEntityStore, JsonFormatTypes, JsonLazyRef, JsonMap, JsonMedia, JsonOperation, JsonParameter, JsonParameterTypes, JsonRequestBody, JsonResponse, JsonSchema, Location, MapOf, Max, MaxItems, MaxLength, MaxProperties, Maximum, Min, MinItems, MinLength, MinProperties, Minimum, MultipleOf, Name, Nullable, OneOf, OperationId, OperationMethods, OperationPath, Optional, Partial, Path, Pattern, Produces, Property, ReadOnly, Redirect, Required, RequiredGroups, Returns, Schema, Security, SpecTypes, Status, Summary, Tags, TimeFormat, Title, UniqueItems, Uri, Url, WriteOnly, allOf, alterIgnore, any, anyOf, array, boolean, date, datetime, defineStatusModel, email, formatParameterType, from, getGenericsMap, getJsonSchema, getJsonType, getOperationsStores, getProperties, getPropertiesStores, getSpec, getStatusConstant, getStatusMessage, getStatusModel, integer, isParameterType, isRedirectionStatus, isSuccessStatus, lazyRef, map, mapGenericsOptions, mapHeaders, mergeSpec, number, object, oneOf, parameters, popGenerics, schemesReducer, security, set, string, tagsReducer, time, uri, url };
