...
exports.ErrorFilter = class ErrorFilter {
    ...
    response.setHeaders(this.getHeaders(error)).status(err.status).contentType("application/json").body(env === core.Env.PROD ? "InternalServerError" : err);
  }
...
Object.values(Exceptions__namespace).forEach(target => {
  if (target !== Exceptions.Exception && target.STATUS) {
    if (target.STATUS > 302) {
      const name = schema.getStatusConstant(target.STATUS);
      schema.from(target).properties({
        name: schema.string().required().example(name).default(name).description("The error name"),
        status: schema.number().required().example(target.STATUS).default(target.STATUS).description("The status code of the exception")
      });
