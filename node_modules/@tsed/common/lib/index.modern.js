import { DIConfiguration, ProviderType, ProviderScope, Injectable, InjectorService, Configuration, Inject, Constant, Scope, DIContext, Provider, Module, setLoggerLevel, GlobalProviders, registerProvider, createContainer, Container, getConfiguration, registerController, DITest } from '@tsed/di';
export * from '@tsed/di';
...
import { Env, AnyToPromise, AnyToPromiseStatus, nameOf, Store, descriptorOf, classOf, prototypeOf, deepMerge, isFunction, Enumerable, DecoratorTypes, isStream, isBoolean, isNumber, isString, uniq, toMap, constructorOf, StoreMerge, Metadata, useDecorators, useMethodDecorators, isArrayOrArrayClass, isClass } from '@tsed/core';
import { PerfLogger } from '@tsed/perf';
import { Exception, NotAcceptable } from '@tsed/exceptions';
import { JsonEntityStore, getOperationsStores, JsonEntityComponent, OperationMethods, getProperties, Returns, Consumes, JsonEntityFn, Produces } from '@tsed/schema';
export { Location, Redirect } from '@tsed/schema';
...
class AnyToPromiseWithCtx extends AnyToPromise {
  ...
  isDone() {
    ...
    if (!($ctx != null && $ctx.isDone())) {
      if ($ctx != null && $ctx.request.isAborted() || $ctx != null && $ctx.response.isDone()) {
        this.destroy();
...
let PlatformHandler = class PlatformHandler {
  ...
  compileHandler(metadata) {
    ...
    if (metadata.type === HandlerType.CTX_FN) {
      return async function (scope) {
        // @ts-ignore
        return _this.onCtxRequest(_extends({}, scope, {
    ...
    return async function (scope) {
      const handler = await promise; // @ts-ignore

      return _this.onRequest(_extends({}, scope, {
        handler,
        metadata
      }));
    };
  ...
  async onRequest(requestOptions) {
    ...
    const {
      ...
      err,
      handler
    } = requestOptions; // istanbul ignore next

    if (!$ctx) {
      $log.error(`Endpoint ${metadata.toString()} is called but the response is already send to your consumer. Check your code and his middlewares please!`);
      return;
    }

    const resolver = new AnyToPromiseWithCtx({
      $ctx,
      err
    });
    return this.injector.runInContext($ctx, async function () {
      try {
        ...
        if (state === AnyToPromiseStatus.RESOLVED) {
          ...
          return await _this2.onSuccess($ctx.data, requestOptions);
        }
      } catch (er) {
        return _this2.onError(er, requestOptions);
      }
    });
  }
  ...
  async onSuccess(data, requestOptions) {
    const {
      ...
      $ctx,
      next
    } = requestOptions; // TODO see this control is necessary
    // istanbul ignore next
  ...
  getDefaultArgs(metadata) {
    return async function (scope) {
      return [metadata.hasErrorParam && scope.err, scope.$ctx.request.request, scope.$ctx.response.response, metadata.hasNextFunction && scope.next].filter(Boolean);
    };
  }
...
PlatformRequest = __decorate([Injectable(), Scope(ProviderScope.INSTANCE), __metadata("design:paramtypes", [Object])], PlatformRequest);
...
PlatformResponse = PlatformResponse_1 = __decorate([Injectable(), Scope(ProviderScope.INSTANCE), __metadata("design:paramtypes", [Object])], PlatformResponse);

const _excluded$3 = ["event", "endpoint", "ignoreUrlPatterns", "ResponseKlass", "RequestKlass"];
...
class PlatformContext extends DIContext {
  ...
  async destroy() {
    var _this$request, _this$request$request;

    await super.destroy();
    (_this$request = this.request) == null ? true : (_this$request$request = _this$request.request) == null ? true : delete _this$request$request.$ctx;
...
const _excluded$2 = ["reqIdBuilder"];
...
function createContext(injector) {
  var _injector$getProvider, _injector$getProvider2;

  const ResponseKlass = (_injector$getProvider = injector.getProvider(PlatformResponse)) == null ? void 0 : _injector$getProvider.useClass;
  const RequestKlass = (_injector$getProvider2 = injector.getProvider(PlatformRequest)) == null ? void 0 : _injector$getProvider2.useClass;
  const platformViews = injector.get(PlatformViews);

  const _injector$settings$lo = injector.settings.logger,
        {
    reqIdBuilder = defaultReqIdBuilder
  } = _injector$settings$lo,
        loggerOptions = _objectWithoutPropertiesLoose(_injector$settings$lo, _excluded$2);

  return async event => {
    const ctx = new PlatformContext(_extends({
      event,
...
class PlatformTest extends DITest {
  ...
  static createRequest(options = {}) {
    return _extends({
      ...
      accepts(mime) {
        return require("accepts")(this).types([].concat(mime));
      }
  ...
  static createRequestContext(options = {}) {
    var _options$request, _options$event, _options$response, _options$event2;

    const event = {
      request: (options == null ? void 0 : (_options$request = options.request) == null ? void 0 : _options$request.request) || (options == null ? void 0 : (_options$event = options.event) == null ? void 0 : _options$event.request) || PlatformTest.createRequest(),
      response: (options == null ? void 0 : (_options$response = options.response) == null ? void 0 : _options$response.response) || (options == null ? void 0 : (_options$event2 = options.event) == null ? void 0 : _options$event2.response) || PlatformTest.createResponse()
    };
    const ctx = new PlatformContext(_extends({
      id: "id",
    ...
    ctx.response.platformViews = PlatformTest.get(PlatformViews);
    return ctx;
  }
...
export { AcceptMime, AcceptMimesMiddleware, All, AnyToPromiseWithCtx, Controller, ControllerProvider, ConverterService, Delete, EndpointFn, EndpointInfo, EndpointMetadata, Err, Get, GlobalAcceptMimesMiddleware, HandlerMetadata, HandlerType, Head, HttpServer, HttpsServer, MulterFileSize, MulterOptions, MultipartFile, Next, Options, PLATFORM_ROUTER_OPTIONS, Patch, Platform, PlatformAcceptMimesMiddleware, PlatformApplication, PlatformBuilder, PlatformConfiguration, PlatformContext, PlatformControllerBuilder, PlatformHandler, PlatformLogMiddleware, PlatformModule, PlatformMulterMiddleware, PlatformRequest, PlatformResponse, PlatformRouteDetails, PlatformRouter, PlatformTest, Post, PropertyMetadata, Put, ROUTER_OPTIONS, Req, Request, Res, Response, ResponseData, bindEndpointMiddleware, createContext, createHttpServer, createHttpsServer, createInjector, createPlatformApplication, listenHttpServer, listenHttpsServer, listenServer, loadInjector, mapReturnedResponse, printRoutes, useCtxHandler };
