...
})(exports.ProviderScope || (exports.ProviderScope = {}));
...
class Provider {
  constructor(token, options = {}) {
    ...
    this.type = exports.ProviderType.PROVIDER;
    this.provide = token;
    this.useClass = token;
  ...
  set provide(value) {
    if (value) {
      _classPrivateFieldLooseBase(this, _provide)[_provide] = core.getClassOrSymbol(value);
      _classPrivateFieldLooseBase(this, _tokenStore)[_tokenStore] = _classPrivateFieldLooseBase(this, _store)[_store] = core.Store.from(value);
    }
  }
  ...
  set useClass(value) {
    if (core.isClass(value)) {
      this._useClass = core.classOf(value);
      _classPrivateFieldLooseBase(this, _store)[_store] = core.Store.from(value);
    }
  }
  ...
  get name() {
    return core.nameOf(this.provide);
  }
  ...
  get scope() {
    if (this.isAsync()) {
      return exports.ProviderScope.SINGLETON;
    }
  ...
  clone() {
    return new (core.classOf(this))(_classPrivateFieldLooseBase(this, _provide)[_provide], this);
  }

  toString() {
    return ["Token", this.name, this.useClass && core.nameOf(this.useClass), this.useFactory && "Factory", this.useValue && "Value", this.useAsyncFactory && "AsyncFactory"].filter(Boolean).join(":");
  }
...
class GlobalProviderRegistry extends Map {
   ...
   */
  get(key) {
    return super.get(core.getClassOrSymbol(key));
  }
  /**
  ...
  has(key) {
    return super.has(core.getClassOrSymbol(key));
  }
  /**
  ...
  set(key, metadata) {
    super.set(core.getClassOrSymbol(key), metadata);
    return this;
  }
  /**
   *
  ...
  delete(key) {
    return super.delete(core.getClassOrSymbol(key));
  }
  ...
  createIfNotExists(key, options) {
    const type = options.type || exports.ProviderType.PROVIDER;
...
exports.InjectablePropertyType = void 0;
...
})(exports.InjectablePropertyType || (exports.InjectablePropertyType = {}));
...
class ContextLogger {
  constructor(logger$1, {
    ...
    _classPrivateFieldLooseBase(this, _level)[_level] = logger.levels()[level.toUpperCase()] || logger.levels().ALL;
    this.maxStackSize = maxStackSize;
  }

  info(obj) {
    this.run(logger.levels().INFO, obj, obj => this.minimalRequestPicker(this.getData(obj)));
    return this;
  }

  debug(obj, withRequest = true) {
    this.run(logger.levels().DEBUG, obj, obj => {
  ...
  warn(obj) {
    this.run(logger.levels().WARN, obj, obj => this.completeRequestPicker(this.getData(obj)));
    return this;
  }

  error(obj) {
    this.run(logger.levels().ERROR, obj, obj => this.completeRequestPicker(this.getData(obj)));
    return this;
  }

  trace(obj) {
    this.run(logger.levels().TRACE, obj, obj => this.completeRequestPicker(this.getData(obj)));
    return this;
  }
...
function Scope(scope = exports.ProviderScope.REQUEST) {
  return core.StoreSet("scope", scope);
}
...
GlobalProviders.createRegistry(exports.ProviderType.CONTROLLER, Provider);
/**
...
const registerValue = (provider, value) => {
  ...
  provider = Object.assign({
    scope: exports.ProviderScope.SINGLETON,
    useValue: value
  }, provider, {
    type: exports.ProviderType.VALUE
  });
  GlobalProviders.merge(provider.provide, provider);
};
/**
...
const registerController = GlobalProviders.createRegisterFn(exports.ProviderType.CONTROLLER);
...
function Inject(symbol, onGet) {
  return (target, propertyKey, descriptor) => {
    const bindingType = core.decoratorTypeOf([target, propertyKey, descriptor]);

    switch (bindingType) {
      case core.DecoratorTypes.PARAM:
      case core.DecoratorTypes.PARAM_CTOR:
        if (symbol) {
          const paramTypes = core.Metadata.getParamTypes(target, propertyKey);
          paramTypes[descriptor] = symbol;
          core.Metadata.setParamTypes(target, propertyKey, paramTypes);
        }

        break;

      case core.DecoratorTypes.PROP:
        core.Store.from(target).merge(INJECTABLE_PROP, {
          [propertyKey]: {
            bindingType,
            propertyKey,
            onGet,
            useType: symbol || core.Metadata.getType(target, propertyKey)
          }
        });
        break;
      ...
      default:
        throw new core.UnsupportedDecoratorType(Inject, [target, propertyKey, descriptor]);
    }
  };
}
...
function Constant(expression, defaultValue) {
  return (target, propertyKey) => {
    core.Store.from(target).merge(INJECTABLE_PROP, {
      [propertyKey]: {
        bindingType: exports.InjectablePropertyType.CONSTANT,
        propertyKey,
        expression,
        defaultValue
      }
...
function Value(expression, defaultValue) {
  return (target, propertyKey) => {
    core.Store.from(target).merge(INJECTABLE_PROP, {
      [propertyKey]: {
        bindingType: exports.InjectablePropertyType.VALUE,
        propertyKey,
        expression,
        defaultValue
      }
...
function Intercept(interceptor, options) {
  return (target, propertyKey, descriptor) => {
    core.Store.from(target).merge(INJECTABLE_PROP, {
      [propertyKey]: {
        bindingType: exports.InjectablePropertyType.INTERCEPTOR,
        propertyKey,
        useType: interceptor,
        options
      }
...
function Interceptor() {
  return Injectable({
    type: exports.ProviderType.INTERCEPTOR
  });
}
  ...
  return (...args) => {
    switch (core.decoratorTypeOf(args)) {
      case core.DecoratorTypes.CLASS:
        core.StoreSet("configuration", configuration)(args[0]);
        break;
...
function Module(options = {}) {
  ...
  return core.useDecorators(Configuration(configuration), Injectable({
    type: exports.ProviderType.MODULE,
    scope: exports.ProviderScope.SINGLETON,
    imports,
    deps,
    injectable: false,
...
function Opts(target, propertyKey, index) {
  Scope(exports.ProviderScope.INSTANCE)(core.classOf(target));
...
function UseOpts(options) {
  return (target, propertyKey, index) => {
    const bindingType = core.decoratorTypeOf([target, propertyKey, index]);

    switch (bindingType) {
      case core.DecoratorTypes.PARAM_CTOR:
        core.Store.from(target).merge(`${DI_PARAM_OPTIONS}:${index}`, options);
        break;
      ...
      default:
        throw new core.UnsupportedDecoratorType(UseOpts, [target, propertyKey, index]);
    }
  };
}
...
class InjectionError extends Error {
  constructor(token, origin) {
    super(core.isString(origin) ? origin : "");
    this.name = "INJECTION_ERROR";
    this.tokens = [];
    this.tokens = [token];

    if (origin) {
      if (core.isString(origin)) {
        this.origin = {
          message: origin,
          stack: this.stack
        };
...
function printDependencyInjectionError(token, options) {
  let erroredArg = "";
  const args = core.getConstructorArgNames(token).map((arg, index) => {
    if (options.index === index) {
      erroredArg = arg;
      arg = chalk__default['default'].red(arg);
    }

    return `${arg}: ${core.nameOf(options.deps[index])}`;
  }).join(", ");
  const signature = core.nameOf(token) + "->constructor(" + args + ")";
  const indexOf = signature.indexOf(erroredArg) - 5;

  const drawline = indexOf => " ".repeat(indexOf) + chalk__default['default'].red("^" + "‾".repeat(erroredArg.length - 1));
...
class DIConfiguration {
  constructor(initialProps = {}) {
    ...
    return core.proxyDelegation(this, {
      ownKeys(target) {
  ...
  setRaw(propertyKey, value) {
    core.setValue(this.map, propertyKey, value);
    return this;
  }
  /**
   *
  ...
  getRaw(propertyKey, defaultValue) {
    const value = core.getValue(this.map, propertyKey);

    if (value !== undefined) {
      return value;
    }

    return core.getValue(this.default, propertyKey, defaultValue);
  }
  ...
  merge(obj) {
    Object.entries(obj).forEach(([key, value]) => {
      ...
      value = core.deepMerge(value, originalValue);
  ...
  resolve(value) {
    if (typeof value === "object" && value !== null) {
      if (![Array, Object].includes(core.classOf(value))) {
        return value;
      }
    ...
    if (typeof value === "string") {
      const replacer = (match, key) => core.getValue(this.map, key);
  ...
  build() {
    ...
    this.get = this.getRaw = (propertyKey, defaultValue) => core.getValue(this.map, propertyKey, defaultValue);
  }
...
exports.InjectorService = InjectorService_1 = class InjectorService extends Container {
  constructor() {
    super();
  ...
  scopeOf(provider) {
    return provider.scope || this.scopes[provider.type] || exports.ProviderScope.SINGLETON;
  }
  /**
  ...
  get(token, options = {}) {
    ...
    const instance = (_super$get = super.get(core.getClassOrSymbol(token))) == null ? void 0 : _super$get.instance;
  ...
  has(token) {
    return super.has(core.getClassOrSymbol(token)) && this.get(token) !== undefined;
  }
  /**
  ...
  invoke(token, locals = new LocalsContainer(), options = {}) {
    ...
    switch (this.scopeOf(provider)) {
      case exports.ProviderScope.SINGLETON:
        if (!this.has(token)) {
      ...
      case exports.ProviderScope.REQUEST:
        instance = this.resolve(token, locals, options);
        locals.set(token, instance);
        break;

      case exports.ProviderScope.INSTANCE:
  ...
  loadSync(locals = new LocalsContainer()) {
    for (const [, provider] of this) {
      if (!locals.has(provider.token) && this.scopeOf(provider) === exports.ProviderScope.SINGLETON) {
  ...
  resolveConfiguration() {
    ...
    super.forEach(provider => {
      if (provider.configuration) {
        Object.entries(provider.configuration).forEach(([key, value]) => {
          value = mergedConfiguration.has(key) ? core.deepMerge(mergedConfiguration.get(key), value) : core.deepClone(value);
    ...
    mergedConfiguration.forEach((value, key) => {
      this.settings[key] = core.deepMerge(value, this.settings[key]);
    });
    this.settings.build();
  ...
  bindInjectableProperties(instance, locals, options) {
    const properties = core.ancestorsOf(core.classOf(instance)).reduce((properties, target) => {
      const store = core.Store.from(target);
      return _extends({}, properties, store.get(INJECTABLE_PROP) || {});
    }, {});
    Object.values(properties).forEach(definition => {
      switch (definition.bindingType) {
        case exports.InjectablePropertyType.METHOD:
          this.bindMethod(instance, definition);
          break;

        case exports.InjectablePropertyType.PROPERTY:
          this.bindProperty(instance, definition, locals, options);
          break;

        case exports.InjectablePropertyType.CONSTANT:
          this.bindConstant(instance, definition);
          break;

        case exports.InjectablePropertyType.VALUE:
          this.bindValue(instance, definition);
          break;

        case exports.InjectablePropertyType.INTERCEPTOR:
  ...
  bindMethod(instance, {
    ...
    const target = core.classOf(instance);
    const originalMethod = instance[propertyKey];
    const deps = core.Metadata.getParamTypes(core.prototypeOf(target), propertyKey);
  ...
  bindConstant(instance, {
    ...
    const clone = o => {
      if (o) {
        return Object.freeze(core.deepClone(o));
      }
  ...
  resolve(target, locals, options = {}) {
    ...
    try {
      const invokeDependency = parent => (token, index) => {
        ...
        return core.isInheritedFrom(token, Provider, 1) ? provider : this.invoke(token, locals, {
          parent
        });
    ...
    if (instance === undefined) {
      throw new InjectionError(token, `Unable to create new instance from undefined value. Check your provider declaration for ${core.nameOf(token)}`);
    }
  ...
  mapInvokeOptions(token, locals, options) {
    ...
    if (provider.useValue !== undefined) {
      construct = () => core.isFunction(provider.useValue) ? provider.useValue() : provider.useValue;
      ...
      // useClass
      isBindable = true;
      deps = deps || core.Metadata.getParamTypes(provider.useClass);
    ...
    return {
      token,
      scope: scope || core.Store.from(token).get("scope") || exports.ProviderScope.SINGLETON,
      deps: deps || [],
      imports: imports || [],
      isBindable,
      construct,
...
exports.InjectorService = InjectorService_1 = tslib.__decorate([Injectable({
  scope: exports.ProviderScope.SINGLETON,
  global: true
}), tslib.__metadata("design:paramtypes", [])], exports.InjectorService);

registerProvider({
  provide: logger.Logger,
  deps: [exports.InjectorService],
...
class DITest {
  ...
  static createInjector(settings = {}) {
    const injector = new exports.InjectorService();
    injector.logger = logger.$log; // @ts-ignore
  ...
  static invoke(target, providers = []) {
    ...
    locals.set(exports.InjectorService, DITest.injector);
  ...
  static configure(settings = {}) {
    return _extends({}, settings, {
      env: core.getValue(settings, "env", core.Env.TEST),
      logger: _extends({}, core.getValue(settings, "logger", {}), {
        level: core.getValue(settings, "logger.level", "off")
      })
    });
  }
...
function getConfiguration(module, configuration = {}) {
  const store = core.Store.from(module).get("configuration") || {};

  const config = _extends({}, store, configuration);

  if (store.mount && configuration.mount) {
    config.mount = core.deepMerge(store.mount, configuration.mount);
  }
...
exports.DI_PARAMS = DI_PARAMS;
exports.DI_PARAM_OPTIONS = DI_PARAM_OPTIONS;
exports.GlobalProviderRegistry = GlobalProviderRegistry;
exports.GlobalProviders = GlobalProviders;
exports.INJECTABLE_PROP = INJECTABLE_PROP;
